...
public class TreeTraversal {
	...
	public static int helperSum(Node root){
		...
		res += helperSum(root.left);
		res += helperSum(root.right);
	...
	public boolean isCousins(Node root, int x, int y) {
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i < size; i++){
                ...
                if(temp.data == x) foundA = true;
                if(temp.data == y) foundB = true;
                Node left = temp.left;
                Node right = temp.right;
                if((left != null && left.data == x) && (right != null && right.data == y)){
                    return false;
                }
                if((left != null && left.data == y) && (right != null && right.data == x)){
                    return false;
                }
                if(left != null){
                   queue.add(left);
	...
	public static Map<Integer, Integer> depth;
	public static Map<Integer, Node> parent;

    public static boolean isCousinsHashMap(Node root, int x, int y) {
        depth = new HashMap<>();
        parent = new HashMap<>();
        dfs(root, null);
